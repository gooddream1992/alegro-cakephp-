...
class UserHelper extends Helper {
    ...
    function checkRoomAvailable($id, $search_room_count, $hotel_check_in, $locations, $hotel_check_out) {

        $table = TableRegistry::get('ExtranetsUserPropertyBed');
        $property_rooms = $table->find('all')->where(['id' => $id])->first();
        ...
        $rm_count = 0;
        foreach ($chk_already_book_rooms_count_details as $rm_det) {
            if ($rm_det['room_id'] == $id) {
                $rm_count = $rm_det['total_rooms_booked'];
            }
        }
        if (($property_rooms->room_count - $rm_count) >= $search_room_count) {
            return "Booing_Available";
        }
        return "Booing_Not_Available";
    }

    public function restRoomCount($room_id, $search_room_count, $hotel_check_in, $hotel_check_out) {
        $table = TableRegistry::get('ExtranetsUserPropertyBed');
        $property_rooms = $table->find('all')->where(['id' => $room_id])->first();

        $conn = ConnectionManager::get('default');
        $ser_hotel_ck_in_dt = date("Y-m-d", strtotime(str_replace('/', '-', $hotel_check_in)));
        $ser_hotel_ck_out_dt = date("Y-m-d", strtotime(str_replace('/', '-', $hotel_check_out)));
        $chk_already_book_rooms_count_details = $conn->execute('SELECT SUM(numb_rooms) AS total_rooms_booked,room_id,property_id FROM hotel_booking_details WHERE ("' . $ser_hotel_ck_in_dt . '" BETWEEN hotel_booking_details.check_in AND hotel_booking_details.check_out OR "' . $ser_hotel_ck_out_dt . '" BETWEEN hotel_booking_details.check_in AND hotel_booking_details.check_out) AND payment_status IN (1,3,6)  GROUP BY room_id')->fetchAll('assoc');
        $count = 0;
        foreach ($chk_already_book_rooms_count_details as $rm_det) {
            if ($rm_det['room_id'] == $room_id) {
                $count = $rm_det['total_rooms_booked'];
            }
        }

        return $property_rooms->room_count - $count;
    }

    function checkRoomAvailableDate($id, $search_room_count, $hotel_check_in, $locations) {

        $table2 = TableRegistry::get('ExtranetsUserPropertyAvailability');
        $room_date_check = $table2->find('all')->where(['sub_id' => $id])->first();

        if (empty($room_date_check->blocked_date_value)) {
            return "Booing_Available";
        }
        //echo $hotel_check_in;pj($room_date_check->blocked_date_value);
        //pj(in_array(date("Y/m/d", strtotime(str_replace('/', '-', $hotel_check_in))), explode(',', $room_date_check->blocked_date_value)));
    ...
    function getSubbedbooking($proId, $mainBedId, $temp_cookie = null) {
        $table = TableRegistry::get('ExtranetsUserPropertyBed');
        $result_property = $table->find('all')->where(['id' => $mainBedId])->count();
        $query = '';
        if ($result_property > 0) {
            $query = $table->find('all')->where(['id' => $mainBedId])->first();
        }
    ...
    function chkhotelproAsc($result_property) {
        $table = TableRegistry::get('ExtranetsUserPropertyPricing');
        $boots_property = $table->find('all')->where(['boost_price' => 0, 'property_id IN' => $result_property->extract('id')->toArray()])->order(['price_main' => 'ASC'])->group('property_id');
    ...
    function getHotelpricekey($id = null, $totalsearch = 0) {
        $table = TableRegistry::get('ExtranetsUserPropertyPricing');
        $result_property = $table->find('all')->where(['property_id' => $id]);
        $min_val = null;
        $min_key = null;
        ...
        return array_search (min(@$price2), $price2);
    }
    ...
    function getSinglkeHotelfixprice($id = null, $totalsearch = 0) {
        $table = TableRegistry::get('ExtranetsUserPropertyPricing');
        $result_property = $table->find('all')->where(['sub_id' => $id])->first();
        return $result_property->fix_price;
    }
    ...
    function propertyPeople($id = null, $guest = null) {
        ...
        $queryx = $table->find('all')->select(['wid' => 'id', 'main_price' => 'MIN(price_main)', 'people' => 'people'])->where(['property_id' => $id])->first();
    ...
    function propertyBedCount($id = null, $guest = null) {
        ...
        $queryx = $table1->find('all')->select(['main_price' => 'MIN(price_main)', 'people' => 'people', 'sub_id' => 'sub_id'])->where(['property_id' => $id])->first();
        // pj($queryx);
        $table = TableRegistry::get('ExtranetsUserPropertyBed');
...
//pj( $query);
    ...
    function propertyEntireSearchAmenities($id = null) {
        $table = TableRegistry::get('ExtranetsUserPropertyAmenities');
        $query = $table->find('all')->where(['property_id' =>$id])->first();
        return $query->Top;
    }
    ...
    function propertyAmenities($id = null, $guest = null) {
        ...
        $query = $table->find('all')->where(['sub_id' => $this->getHotelpricekey($id)])->first();
        return $query->Top;
    }
    ...
    function getDataForEntiterProp($id = null) {
        ...
        $property->hasOne('pricing', ['className' => 'ExtranetsUserPropertyPricing', 'foreignKey' => 'property_id']);
        //$property->hasOne('description', ['className' => 'ExtranetsUserPropertyDescription', 'foreignKey' => 'property_id']);
        $result_property = $property->find('all')->contain(['pricing'])->where(['ExtranetsUserProperty.id' => $id])->first();
        // pj($result_property); exit;
        return $result_property;
    }
    ...
    function propertyEntireAmenities($pro_id = null) {
        $table = TableRegistry::get('ExtranetsUserPropertyAmenities');
        $query = $table->find('all')->where(['property_id' => $pro_id, 'sub_id' => 1])->first();
        return $query;
    }
    ...
    function getCancelationEntireProperty($property_id = null) {
        $table = TableRegistry::get('ExtranetsUserPropertyAvailability');
        $query = $table->find('all')->where(['property_id' => $property_id])->first();
        return $query->cancellation;
    }
    ...
    function getHotelCancellation($room_id) {
        $property = TableRegistry::get('ExtranetsUserPropertyAvailability');
        $price = $property->find('all')->where(['sub_id' => $room_id])->first();
        return $price->cancellation;
    }

    function getUserPic4Review($booking_id = null) {
        if ($booking_id == null) {
            return 'img/avatar1.jpg';
        } else {
            $booking_details = TableRegistry::get('HotelBookingDetails');
            $user_id = $booking_details->find('all')->where(['booking_no' => $booking_id])->first()->user_id;
            $user_details_tb = TableRegistry::get('UserDetails');
            $user_data = $user_details_tb->find('all')->where(['user_id' => $user_id])->first();
            if (!empty($user_data)) {
                if (!empty($user_data->profile_photo)) {
                    return 'files/photos/' . $user_data->profile_photo;
                } else {
                    return 'img/avatar1.jpg';
                }
            } else {
    ...
    function getBookingRoomType($id = null) {
        $booking_details = TableRegistry::get('ExtranetsUserProperty');
        $bookType = $booking_details->find('all')->where(['id' => $id])->first();
        return $bookType->booking_type;
    }

    function getPropertyType($id = null) {
        $booking_details = TableRegistry::get('ExtranetsUserProperty');
        $bookType = $booking_details->find('all')->where(['id' => $id])->first();
        return $bookType->property_type;
    }
