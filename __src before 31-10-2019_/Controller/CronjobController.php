...
class CronjobController extends AppController {
    ...
    public function beforeFilter(Event $event) {
        $this->Auth->allow(['beforeCheckin', 'chkPayment', 'paym', 'validPay', 'createRef', 'cancelPayment','chkHotelServiceFee']);
    }
    ...
    public function cancelPayment() {
        ...
        $alldata = $this->HotelBookingDetails->find('all')->where(['payment_status' => 1]);
        foreach ($alldata as $val) {
            if ($curtim >= date_format($val->end_datetime, 'Y-m-d H:i')) {
                ...
                $this->HotelBookingDetails->updateAll(['payment_status' => 2], ['id' => $val->id]);
            }
        }
