...
<style>
    ...
    .radio-besic{ position: relative; padding-left: 37px !important;}
.radio-besic input{ position: absolute; opacity: 0; }
.checkmark {
    position: absolute;
    top: 0;
    left: 0;
...
.radio-besic input:checked ~ .checkmark {
            ...
            <div class="col-md-10">
                <div class="head-section">
                    ...
                    <div class="selection-section">
                        <?php if (!empty($propertyData->property_type) && in_array($propertyData->property_type, ['Apartment', 'Villa'])) { ?>
                            <script>
                                $(document).ready(function() {
                                    $('.booking_type').show();
                                });
                        ...
                        ?> id="radio1" value="Apartment">
                            ...
                            <li>
                                <input type="radio" name="property_type" id="radio2" <?php if (@$propertyData->property_type == "Hotel") { ?>checked="" <?php
                                } elseif (empty($propertyData->property_type)) {
                    ...
                    <div class="booking_type">
                        <h2 style="margin-top: 29px;"><?php echo __('What will guests book?'); ?></h2>
                        <div class="quantity-section" style="padding-bottom: 0px;">
                            <div class="panel-body cancellation-policy-required">
                                <div class="m-t-1 form-group">
                                    <div class="regular-radio flexible-cancellation-policy-radio-selected radio">
                                        <label class="radio-besic" title="">
                                            <input name="booking_type" type="radio" value="1" <?php
                                            if (@$propertyData->booking_type == 1) {
                                                echo "checked";
                                            }
                                            ?> >
                                            ...
                                            <div class="p-l-2">
                                                <b>
                                                    <div><?php echo __('Entire Place'); ?></div>
                                                </b>
                                                <div><?php echo __('Guests have the whole place to themselves. This usually includes a bedroom, a bathroom and a kitchen.'); ?></div>
                                            </div>
                                        </label>
                                    </div>
                                    <div class="regular-radio moderate-cancellation-policy-radio radio">
                                        <label class="radio-besic" title="">
                                            <input name="booking_type" type="radio" value="2" <?php
                                            if ((@$propertyData->booking_type == 2) || (empty($propertyData->booking_type))) {
                                                echo "checked";
                                            }
                                            ?>>
                                            <span class="checkmark"></span>
                                            <div class="p-l-2">
                                                <b>
                                                    <div><?php echo __('Private Room'); ?></div>
                                                </b>
                                                <div><?php echo __('Guests have their own private room for sleeping, but other areas could be shared.'); ?></div>
                                            </div>
                                        </label>
                                    </div>
                                </div>
...
<script>
    ...
    function getBedrooms(val) {
        var total = 1;
        var property_book_type = $('input[name="property_type"]:checked').val();
        var room__booking_type = $('input[name="booking_type"]:checked').val();
        if (val == '') {
        ...
        $.ajax({
            ...
            data: {total: total, id: '<?= @$propertyData->id ?>', property_book_type: property_book_type, room__booking_type: room__booking_type},
            dataType: 'html',
...
<script>
    ...
    $('input[name="property_type"]').click(function() {
        ...
        if ($.inArray($(this).val(), ['Apartment', 'Villa']) !== -1) {
            $("input[name='booking_type'][value='2']").prop('checked', false);
            $("input[name='booking_type'][value='1']").prop('checked', true);
            $('.booking_type').show();
        } else {
            $("input[name='booking_type'][value='1']").prop('checked', false);
            $("input[name='booking_type'][value='2']").prop('checked', true);
            $('.booking_type').hide();
        }
        getBedrooms(1);
    });
