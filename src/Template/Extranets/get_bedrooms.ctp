<!--Already exits --->
<?php
//echo $data['property_book_type'];
if (!empty(@$bedDet)) {
    $i = 1;
    // echo $data['property_book_type'] . '-1';
        ...
        <div id="ulcontent<?= $dats->id; ?>">
            <h5>
                <div class="box-border" style="margin-bottom: 10px;margin-top: 50px;">
                    <?php
                    if (in_array($data['property_book_type'], ['Apartment', 'Villa']) && ($data['room__booking_type'] == 1)) {
                    ...
                    if (!empty(@$dats->room_count)) {
                        echo @$dats->room_count;
                    } else {
                        echo "1";
                    }
                    ?>" <?php
                           if (in_array($data['property_book_type'], ['Apartment', 'Villa']) && ($data['room__booking_type'] == 1)) {
                               echo "readonly style='width:100%;text-align:center;background: #ccc;cursor: not-allowed;'";
                           }
                           ?> >
                           <?php
                           if (in_array($data['property_book_type'], ['Apartment', 'Villa']) && ($data['room__booking_type'] == 1)) {
