    ...
    <div class="container">
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12">
                ...
                <?php
                if (!empty($id)) {
                    $fstHotel = $this->User->getDataForRoom($id);
                    $htl_rm_pric_frea = $this->User->get_featured_fee($fstHotel->property_id);
                }
                $guest = $_GET['adult'] + @$_GET['children'];
                $longDays = $this->User->longDays($fstHotel->property_id);
                $htl_fixrm_pric = $this->User->getSinglkeHotelfixprice($id, $guest);
                ...
                if ($dategap >= $longDays && !empty($longDays)) {
                    $ttfee = @$_GET['rooms'] * ($htl_fixrm_pric * $dategap * $_GET['rooms']);
                } else {
                    $ttfee = @$_GET['rooms'] * ($htl_pri * $dategap * $_GET['rooms']);
                }
                    ...
                    <div class="aparthotel-left">
                        <div class="aparthotel-first">
                            <div class="aparthotel-left-img">
                                ...
                                ?>
                                <img src="<?= HTTP_ROOT . 'img/pro_pic/' . $fstImgUrl; ?>">
                            </div>
                                ...
                                <ul>
                                    ...
                                    <li>
                                        <div class="your-stay-part"><?= __("Your Stay"); ?>
                                            <span>
                                                <?php echo __(@$_GET['rooms']); ?><?php if ($_GET['rooms'] <= 1) { ?> <?php echo __('Room') ?> <?php } else { ?> <?php echo __('Rooms') ?> <?php } ?>, <?= $dategap; ?><?php if ($dategap > 1) { ?> <?php echo __('nights') ?> <?php } else { ?> <?php echo __('night') ?><?php } ?>
                                            </span>
                                        </div>
                                    </li>
                                </ul>
                        ...
                        <div class="aparthotel-right aparthotel-full" >
                            <div class="aparthotel-right-box"><i class="fa fa-bell"></i><?= __("Only"); ?> <b><?= $Rm_counts = $this->User->restRoomCount($RmDet->id, @$_GET['rooms'], @$_GET['hotel_check_in'], @$_GET['hotel_check_out'])//->room_count;                                                                            ?> <?= ($Rm_counts > 1) ? __('rooms') : __('room'); ?></b> <?= __("left at this price!"); ?><?php
                                if ($Rm_counts == 0) {
                                    $this->Flash->error(__('Rooms not available.'));
                                    header('Location:' . HTTP_ROOT . '404.html');
                                }
                                ?></div>
                            <div class="aparthotel-right-box2">
                                <div class="bedroom-apartment">
                                    <div class="aparthotel-right-text "><p><?= __("Room Type"); ?>:</p><h6><?= $RmDet->bed_name; ?></h6></div>
                                    ...
                                    <h6>
                                        ...
                                        <?php
//pj($aminity);
                                        $aminity = $this->User->propertyRoomAmenities($fstHotel->id);
                                        $aminityx = [];
                                    ...
                                    <h6><?= __($this->User->cancelPolicy($RmDet->id)); ?></h6>
                                </div>
                                <div class="room-cost" >
                                    <ul>
                                        <li><?= __("Price Per Room"); ?>  <span><?php
                                                if (!empty($htl_pri)) {
                                                ...
                                                ?></span></li>
                                        <li><?php echo __(@$_GET['rooms']); ?> <?php if ($_GET['rooms'] <= 1) { ?> <?php echo __('Room') ?> <?php } else { ?> <?php echo __('Rooms') ?> <?php } ?> x <?= $dategap; ?> <?php if ($dategap > 1) { ?> <?= __("nights"); ?>  <?php
                                            } else {
                                                echo __("night");
                                            }
                                            ?>
                                            <span>
                                                ...
                                                if ($dategap >= $longDays && !empty($longDays)) {
                                                    if (!empty($this->request->session()->read("CURRENCY"))) {
                                                        echo $this->request->session()->read("CURRENCY") . ' ' . number_format($this->User->priceConvert($this->request->session()->read("CURRENCY"), $dategap * $htl_fixrm_pric* $_GET['rooms']), 2);
                                                    } else {
                                                        echo 'AOA ' . $this->User->changeFormat(number_format($dategap * $htl_fixrm_pric* $_GET['rooms'], 2));
                                                    }
                                                } else {
                                                    if (!empty($this->request->session()->read("CURRENCY"))) {
                        ...
                        <div class="aparthotel-3rd">
                            <h4><?= __("Name of Guests"); ?> <span>(<?= __("Aged 18 or older"); ?>)</span></h4>
                            <p><?= __("Enter name of guests checking into each rooms. Remember to bring valid photo IDs and credit cards at check-in"); ?></p>
                            <input type="hidden" id="room" name="num_rooms" value="<?= @$_GET['rooms']; ?>">
                            <?php for ($ic = 1; $ic <= @$_GET['rooms']; $ic++) { ?>
                                <label for="fname"><?= __("Room"); ?> <?= $ic; ?><span>*</span></label>
                                <input type="text" id="fname" name="fnm[]" placeholder="<?= __("First Name"); ?>" required>
                                <input type="text" id="lname" name="lnm[]" placeholder="<?= __("Last Name"); ?>" required>
                            <?php } ?>
                        ...
                        <div class="Credit-Card">
                            ...
                            <ul class="cardTipe">
                                ...
                                <?php if (($this->User->getHotelCancellation($RmDet->id) == 'Flexible')) { ?>
                            ...
                            <script>
                                ...
                                function checkImg(val = '', show, hide) {
                                    ...
                                    if (val == "proxypay") {
                                        ...
                                        $('#bookingformdetails').attr('action', '<?= $this->Url->build(["controller" => "Users", "action" => "booking", $id,]); ?>');
                                    } else if (val == "paypal") {
                                        $('#bookingformdetails').removeAttr('action');
                                        $('#bookingformdetails').attr('action', '<?= $this->Url->build(["controller" => "Users", "action" => "order", $id,]); ?>');
                                    } else if (val == "pay-local") {
                                        $('#bookingformdetails').removeAttr('action');
                                        $('#bookingformdetails').attr('action', '<?= $this->Url->build(["controller" => "Users", "action" => "paylocal", $id,]); ?>');
                                }
                                }
                            </script>
                        ...
                        <input type="hidden" name="cancel_policy" value="<?= $this->User->cancelPolicy($RmDet->id); ?>" />
