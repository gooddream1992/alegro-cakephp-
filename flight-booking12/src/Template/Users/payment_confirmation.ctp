 ...
 <div class="wrapper">
  ...
  <div class="event_description">
   ...
   <?php
     ...
     $toEncrypt = PASSWORD . MERCHAN_ID . ACQIRER_ID . $orderID . $formattedPurchaseAmt . CURRENCY;
   ...
   <input type = "hidden" name = "MerID" value="<?php echo MERCHAN_ID ?>"/>
   <input type = "hidden" name ="AcqID" value = "<?php echo ACQIRER_ID ?>"/>
   ...
   <div class="acceptTerms"><input id="termsAccept" type="checkbox" name="terms"> I accept the <a  target="_blank" href="<?php echo HTTP_ROOT . 'terms-conditions' ?>">Terms & Conditions</a></div>

   <a href="<?php echo HTTP_ROOT . 'personal-info' ?>" class="backBtn">Back</a>
   <button type="submit" class="payNow" style="float: right; margin-top: 0px;"name="paynow" id="paynow" value="paynow">Pay Now</button>
...
// Submit form with id function.
  $('#paymentForm').submit(function(e){
   if(!($('#termsAccept').is(':checked'))){
    $('#error_msg').html('<div class="message error" id="e" onclick="this.classList.add(\'hidden\');">Please accept the Terms & Conditions</div>');
    return false;
   }else{
    return true;
   }
