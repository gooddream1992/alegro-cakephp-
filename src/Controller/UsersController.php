...
class UsersController extends AppController {
    ...
    public function beforeFilter(Event $event) {
        $this->Auth->allow(['currency', 'accountAlreadyExists', 'accountConfirmed', 'loginDirect', 'ajaxLogin', 'webrootRedirect', 'login', 'forget', 'index', 'ajaxCheckEmailAvail', 'ajaxRegistration', 'welcomePage', 'confirm', 'forgetPassword', 'changePassword', 'language', 'ajaxSearchResult', 'ajaxOneWayResult', 'ajaxLocations', 'ajaxHotelLocations', 'pagedata', 'pricedatas', 'airportdata', 'fareDetails', 'thankYou', 'routeReview', 'emailvalidate', 'ShareItenerary', 'invoicePdf', 'itineraryPdf', 'mailtests', 'cronJob', 'hotelSearchResult', 'hotelajaxsearchdata', 'preview', 'booking', 'thankYouPrivateOrder', 'order', 'paypalProcess', 'paymentSuccess', 'ajaxCheckEmailStatus', 'hotelAjaxReview', 'fblogin', 'fbreturn', 'googlelogin', 'googleLoginReturn', 'testMailfun', 'hotelBookingPdf', 'paylocal', 'thankYouAtPrivateOrder', 'hotelInvoicePdf', 'previewPrivate', 'previewEntire', 'bookingPrivate', 'bookingEntire', 'thankYouEntireOrder', 'registration', 'resendEmailVerification', 'paylocalentire', 'thankYouAtEntireOrder']);
    }
...
//        $activecount = $active->count();
    ...
    public function hotelSearchResult() {
        ...
        if ($this->request->is('get')) {
            ...
            $all_available_properties_types = [];
            $all_available_booking_types = [];
            ...
            $final_result_property_type = [];
            if (!empty($datas['property_type'])) {
                $htl_property_type = explode(',', $datas['property_type']);
                $final_result_property_type = ['main_tb.property_type IN ' => $htl_property_type];
            }

            $final_result_booking_type = ['main_tb.booking_type IN ' => [1, 2]];
            if (!empty($datas['booking_type'])) {
                $htl_booking_type = explode(',', $datas['booking_type']);
                if (in_array('Entire Place', $htl_booking_type)) {
                    $final_result_booking_type = ['main_tb.booking_type' => 1];
                }
                if (in_array('Private Room', $htl_booking_type)) {
                    $final_result_booking_type = ['main_tb.booking_type' => 2];
                }
                if (in_array('Private Room', $htl_booking_type) && in_array('Entire Place', $htl_booking_type)) {
                    $final_result_booking_type = ['main_tb.booking_type IN ' => [1, 2]];
                }
            }
            ...
            $search_property = $this->ExtranetsUserPropertyLocation->find('all')->contain(['main_tb'])->where(['state' => $locations, 'main_tb.complete_ststus' => 1, 'main_tb.active_ststus' => 1, $final_result_property_type, $final_result_booking_type]);
...
//                pj($search_property->count());
                ...
                $property_room_count = [];
                $all_available_properties_types = array_unique($search_property->extract('main_tb.property_type')->toArray());

                $all_available_booking_types_data = array_unique($search_property->extract('main_tb.booking_type')->toArray());

                if (in_array(1, $all_available_booking_types_data)) {
                    $all_available_booking_types = ['Entire Place'];
                }
                if (in_array(2, $all_available_booking_types_data)) {
                    $all_available_booking_types = ['Private Room'];
                }
                if (in_array(1, $all_available_booking_types_data) && in_array(2, $all_available_booking_types_data)) {
                    $all_available_booking_types = ['Entire Place', 'Private Room'];
                }
...
//pj($intersect); exit;
            ...
            if (!empty($result_property)) {
                if ($result_property->count()) {
                    $property_bed = $this->ExtranetsUserPropertyBed->find('all')->where(['property_id IN' => $result_property->extract('id')->toArray()])->group(['property_id']);
                }
            }
            ...
            if (!empty($property_bed)) {
                ...
                foreach ($property_bed as $bed) {
                    ...
                    //  echo $bed->property_id.',';
                    $property_bedx = $this->ExtranetsUserPropertyBed->find('all')->where(['property_id' => $bed->property_id]);
                    // pj($property_bedx);


                    foreach ($property_bedx as $bedsx) {
                        // pj($bedsx);

                        if (($bedsx->beds == 'single bed') || ($bedsx->beds == 'futon')) {
                            $personv1 = $bedsx->num_bed * 1;
                        }

                        if (($bedsx->beds == 'double bed') || ($bedsx->beds == 'semi double-bed') || ($bedsx->beds == 'queen bed') || ($bedsx->beds == 'semi double-bed') || ($bedsx->beds == 'king bed') || ($bedsx->beds == 'super king bed') || ($bedsx->beds == 'bunk bed') || ($bedsx->beds == 'sofa bed')) {
                            $personv1 = $bedsx->num_bed * 2;
                        }

                        if ($bedsx->room_count > 0) {
                            $person1 = $personv1 * $bedsx->room_count;
                        } else {
                            $person1 = $personv1;
                        }
                        ...
                        // $bedCount[$bedsx->id] = $person;
                        //pj($bedCount);
                        // $sub_person_total = [];

                        $getSubBed = $this->ExtranetsUserPropertySubBeds->find('all')->where(['main_bed_id' => $bedsx->id]);
                        // pj($getSubBed);
                        if ($getSubBed->count() > 0) {
                    ...
                    //   }else if((@$adult + @$children) == $nubedsxd ){
                    //       @$all_beds_count[$bed->property_id][$bed->property_id] = $nubedsxd;
                    //   }

                    if ((@$adult + @$children) <= $person) {

                        $all_beds_count[$bedsx->property_id][$bedsx->id] = $person;
                    }
                }
                $lisArr = [];
                 pj($all_beds_count);
                ...
                if ($all_beds_count) {
                    ...
                    foreach ($all_beds_count as $keyz => $count) {
                        ...
                        $result_property = $this->ExtranetsUserProperty->find('all')->contain(['location', 'description'])->where(['ExtranetsUserProperty.id IN' => $lisArr])->group(['ExtranetsUserProperty.id']);
            ...
            $this->set(compact('result_property', 'country', 'city', 'result_property_count', 'rati_htl', 'search_property_count', 'all_available_properties_types', 'all_available_booking_types'));
        }
    }
    ...
    public function hotelajaxsearchdata() {
        ...
        if ($this->request->is('post')) {
            ...
            if (!empty($data['property_type'])) {
                $htl_property_type = explode('|', $data['property_type']);
                $final_result_property_type = ['main_tb.property_type IN ' => $htl_property_type];
            }

            $final_result_booking_type = ['main_tb.booking_type IN ' => [1, 2]];
            if (!empty($data['booking_type'])) {
                $htl_booking_type = explode('|', $data['booking_type']);
...
//                        }
                ...
                $chk_already_book_rooms_count_details = $conn->execute('SELECT SUM(numb_rooms) AS total_rooms_booked,room_id,property_id FROM hotel_booking_details WHERE "' . $ser_hotel_ck_in_dt . '" BETWEEN hotel_booking_details.check_in AND hotel_booking_details.check_out OR "' . $ser_hotel_ck_out_dt . '" BETWEEN hotel_booking_details.check_in AND hotel_booking_details.check_out AND payment_status=3 AND location="' . $locations . '"')->fetchAll('assoc');
...
//        }
        ...
        if (!empty($data['sortby']) && !empty($all_final_set1)) {
            ...
            if (!empty($data['sortby'] == 'rev')) {
                if (!empty($data['star_rating'])) {
                    $result_property = $this->ExtranetsUserPropertyDescription->find('all')->select(['main_price' => '0', 'property_id' => 'property_id', 'sub_id' => '0'])->where(['rating <=' => $data['star_rating'], 'ExtranetsUserPropertyDescription.property_id IN' => $lisArr]);
                    ...
                    $result_property = $this->ExtranetsUserPropertyDescription->find('all')->select(['main_price' => '0', 'property_id' => 'property_id', 'sub_id' => '0'])->where(['rating IN' => [1, 2, 3, 4, 5], 'ExtranetsUserPropertyDescription.property_id IN' => $lisArr]);
                    $result_property_count = $this->ExtranetsUserPropertyDescription->find('all')->where(['rating IN' => [1, 2, 3, 4, 5], 'ExtranetsUserPropertyDescription.property_id IN' => $lisArr])->order(['rating' => 'DESC'])->count();
                }
            }
        } else {
...
// }
    ...
    public function ajaxRegistration() {
        ...
        if ($this->request->is('post')) {
            ...
            $exitEmail = $this->Users->find('all')->where(['Users.email' => $data['email'], 'Users.type' => 2])->count();
            //echo $exitEmail;exit;
            ...
            if ($exitEmail >= 1) {
                ...
                if ($lastid = $this->Users->save($user)) {
                    ...
                    if ($this->UserDetails->save($userDetailspatch)) {
                        ...
                        if ($LAN == "PT") {
                            $valuez = "value_PT";
                            $subject = "Bem-Vindo ao Alegro";
                            $btn_name = "Confirmar";
                        } else if ($LAN == "FR") {
                            $valuez = "value_FR";
                            $subject = "Bienvenue sur Alegro";
                            $btn_name = "Confirmer";
                        } else {
                            $valuez = "value";
...
//
    ...
    public function forgetPassword() {
        ...
        if ($this->request->is(['post'])) {
            ...
            if ($data['email'] == "") {
                ...
                $users = $this->Users->find('all')->where(['Users.email' => $data['email'], 'Users.type' => 2]);
                $user = $users->first();
                if ($users->count() > 0) {
            ...
            if ($LAN == "PT") {
                ...
                $subject = "Forgot your password?";
					...
					$link = HTTP_ROOT . 'change-password/' . $user->unique_id;
    ...
    public function changePassword($uniq = null) {
        $this->viewBuilder()->layout('default');
        $pageDetails = new \stdClass();
        $pageDetails->meta_title = 'Change Password | Alegro';
		$LAN = $this->request->session()->read("lan");
            if ($LAN == "PT") {
                $valuez = "value_PT";
                $pageDetails->meta_title = "Altere a palavra-passe | Alegro";
            } else if ($LAN == "FR") {
                $valuez = "value_FR";
                $pageDetails->meta_title = "Changer le mot de passe | Alegro";
            } else {
                $valuez = "value";
                $pageDetails->meta_title = "Change Password | Alegro";
            }
...
// you can call any mPDF method via component, for example:
    ...
    public function previewEntire($id = null) {
        ...
        $result_property = $this->ExtranetsUserProperty->find('all')->contain(['availability', 'location', 'pricing', 'description', 'beds', 'amenities', 'photos'])->where(['ExtranetsUserProperty.id IN' => $id])->first();
//        pj($result_property);
        // Checking Booking is available or not
        $ser_hotel_ck_in_dt = date("Y-m-d", strtotime(str_replace('/', '-', $_GET['hotel_check_in'])));
        $ser_hotel_ck_out_dt = date("Y-m-d", strtotime(str_replace('/', '-', $_GET['hotel_check_out'])));

        $check_booking_available = $this->HotelBookingDetails->find('all')->where(['property_id' => $id, 'payment_status' => 3]);

        $conn = ConnectionManager::get('default');
//        echo 'SELECT * FROM hotel_booking_details WHERE property_id="' . $id . '" AND ( "' . $ser_hotel_ck_in_dt . '" BETWEEN hotel_booking_details.check_in AND hotel_booking_details.check_out OR "' . $ser_hotel_ck_out_dt . '" BETWEEN hotel_booking_details.check_in AND hotel_booking_details.check_out ) ';
//        exit;
        $check_booking_available = $conn->execute('SELECT * FROM hotel_booking_details WHERE property_id="' . $id . '" AND ( "' . $ser_hotel_ck_in_dt . '" BETWEEN hotel_booking_details.check_in AND hotel_booking_details.check_out OR "' . $ser_hotel_ck_out_dt . '" BETWEEN hotel_booking_details.check_in AND hotel_booking_details.check_out ) ')->fetchAll('assoc');

//        pj($check_booking_available);
//        exit;

        $booking_available = 1;
        if (!empty(count($check_booking_available))) {
            $booking_available = 0;
        }
//        foreach ($check_booking_available as $h_vals) {
//            if ($ser_hotel_ck_in_dt >= date_format($h_vals->check_in, 'Y-m-d') || $ser_hotel_ck_in_dt >= date_format($h_vals->check_out, 'Y-m-d') || $ser_hotel_ck_out_dt >= date_format($h_vals->check_in, 'Y-m-d') || $ser_hotel_ck_out_dt >= date_format($h_vals->check_out, 'Y-m-d')) {
//                var_dump($ser_hotel_ck_in_dt >= date_format($h_vals->check_in, 'Y-m-d') || $ser_hotel_ck_in_dt >= date_format($h_vals->check_out, 'Y-m-d') || $ser_hotel_ck_out_dt >= date_format($h_vals->check_in, 'Y-m-d') || $ser_hotel_ck_out_dt >= date_format($h_vals->check_out, 'Y-m-d'));
//
//                var_dump($ser_hotel_ck_in_dt >= date_format($h_vals->check_in, 'Y-m-d') && $ser_hotel_ck_in_dt >= date_format($h_vals->check_out, 'Y-m-d'));
//                var_dump($ser_hotel_ck_out_dt >= date_format($h_vals->check_in, 'Y-m-d') && $ser_hotel_ck_out_dt >= date_format($h_vals->check_out, 'Y-m-d'));
//                $booking_available = 1;
//            }
//        }
//        var_dump($booking_available);
////        exit;
        ...
        $this->set(compact('pageDetails', 'result_property', 'city', 'country', 'id', 'proper_loc', 'booking_available'));
    }
    ...
    public function paylocal($id = null) {
        ...
        if ($this->request->is('post')) {
            ...
            $book_link = HTTP_ROOT . 'thank_you_private_order/' . $htl_booking_details_email->room_id . '/' . $htl_booking_details_email->property_id . '/' . $htl_booking_details_email->id;

            $emailTemplate = $this->Settings->find('all')->where(['Settings.name' => 'APPROVED_HOTEL_BOOKING_GUEST'])->first();
            ...
            $this->Custom->sendEmail($to, $from, $subject, $message);
            return $this->redirect(HTTP_ROOT . 'thank_you_at_private_order/' . $id . '/' . $RmDet->property_id . '/' . $gt_htl_id->id . '?' . $dats['qury_tri']);
        }

        exit;
    }

    public function paylocalentire($id = null) {
        $user_id = $_COOKIE['user__id'];
        ...
        if (!empty($id)) {
            ...
            $this->ExtranetsUserProperty->hasOne('description', ['className' => 'ExtranetsUserPropertyDescription', 'foreignKey' => 'property_id']);
            $this->ExtranetsUserProperty->hasMany('photos', ['className' => 'ExtranetsUserPropertyPhotos', 'foreignKey' => 'property_id']);
            $PrtDt = $this->ExtranetsUserProperty->find('all')->contain(['description', 'photos'])->where(['ExtranetsUserProperty.id' => $id])->first();
        }
        if ($this->request->is('post')) {
            $dats = $this->request->data();
            $roomBookedUser = '';
            ...
            $refId = $htl_booking_details_email->refer_id;
            $book_link = HTTP_ROOT . 'thank_you_at_entire_order/' . $htl_booking_details_email->property_id . '/' . $htl_booking_details_email->id;
            ...
            $this->Custom->sendEmail($to, $from, $subject, $message);
            return $this->redirect(HTTP_ROOT . 'thank_you_at_entire_order/' . $id . '/' . $gt_htl_id->id . '?' . $dats['qury_tri']);
        }
...
//            $propertyFee1 = $this->HotelServiceFee->find('all')->where(['id' => 1])->first();
        ...
        if ($this->request->is('post')) {
            ...
            if (!empty($refer_idd)) {
                ...
                $emailTemplate = $this->Settings->find('all')->where(['Settings.name' => 'PENDING_HOTEL_BOOKING_GUEST'])->first();
                $message = $this->Custom->bookingPending($emailTemplate->$valuez, date_format($htl_booking_details_email->booking_dt, 'd M Y'), $htl_booking_details_email->booking_no, $propertyD_email->description->propertyName, $hotel_size, $hotel_guest, $hotel_rooms, $aminity, $price_night, $price_total, $total_night, $guest, $chk_in, $chk_out, $room_num, $room_type, $qlt, $refId, $book_link, $pro_img, $review, $rating);
                ...
                $this->Custom->sendEmail($to, $from, $subject, $message);
                return $this->redirect(HTTP_ROOT . 'thank_you_private_order/' . $id . '/' . $RmDet->property_id . '/' . $gt_htl_id->id . '?' . $dats['qury_tri']);
            } else {
    ...
    public function bookingEntire($id = null) {
...
//                $newR->room_lnm = json_encode($dats['lnm']);
                ...
                $refId = $htl_booking_details_email->refer_id;
                $book_link = HTTP_ROOT . 'thank_you_entire_order/' . $htl_booking_details_email->property_id . '/' . $htl_booking_details_email->id;
                ...
                $this->Custom->sendEmail($to, $from, $subject, $message);
                return $this->redirect(HTTP_ROOT . 'thank_you_entire_order/' . $id . '/' . $gt_htl_id->id . '?' . $dats['qury_tri']);
            } else {
                $this->Flash->error(__('Payment process fail'));
                return $this->redirect(HTTP_ROOT . 'booking_entire/' . $id . '?' . $dats['qury_tri']);
            }
        }
    ...
    public function bookingPrivate($id = null) {
...
//            $propertyFee1 = $this->HotelServiceFee->find('all')->where(['id' => 1])->first();
        ...
        if ($this->request->is('post')) {
            ...
            if (!empty($refer_idd)) {
                ...
                $this->Flash->error(__('Payment process fail'));
                return $this->redirect(HTTP_ROOT . 'booking_private/' . $id . '?' . $dats['qury_tri']);
            }
        }
    ...
    function thankYouPrivateOrder($room_id = null, $pro_id = null, $id = null) {
    ...
    function thankYouEntireOrder($room_id = null, $id = null) {

        $reff = $_COOKIE['refer_id'];
        $htl_booking_details = $this->HotelBookingDetails->find('all')->where(['id' => $id])->first();

        $room_count = $this->ExtranetsUserPropertyBed->find('all')->where(['property_id' => $room_id])->first();
        $pageDetails = $this->Pages->find('all')->where(['Pages.id' => 20])->first();
        ...
        $this->ExtranetsUserProperty->hasMany('photos', ['className' => 'ExtranetsUserPropertyPhotos', 'foreignKey' => 'property_id']);
        $propertyD = $this->ExtranetsUserProperty->find('all')->contain(['availability', 'location', 'description', 'beds', 'amenities', 'photos'])->where(['ExtranetsUserProperty.id IN' => $room_id])->first();

        $this->set(compact('propertyD', 'reff', 'htl_booking_details', 'pro_id', 'id', 'room_id', 'pageDetails'));
    }

    function thankYouAtEntireOrder($room_id = null, $id = null) {
    ...
    function thankYouAtPrivateOrder($room_id = null, $pro_id = null, $id = null) {
    ...
    public function order($id = null) {
        ...
        $success_return = HTTP_ROOT . 'thank_you_private_order/' . $id;
