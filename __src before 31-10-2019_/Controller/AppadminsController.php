...
//    $this->loadComponent('Phpoffice');
    ...
    public function hotelRefundBooking(){
...
//        $monthPurchase = $this->UserBookDetails->find('all')->where(['id' => 14, 'MONTH(purches_date)' => date('m')]);
    ...
    public function index($id = null) {
        ...
        $this->set(compact('bookings_count', 'pass_count', 'revenuessum', 'no_htl_book', 'htl_book_rev', 'htl_nos'));
    }
...
//$this->Custom->sendEmail($to, $from, $subject, $message);
    ...
    public function hotelRevenues() {
        ...
        foreach ($prop_count as $pro_p) {
            ...
            $y_pro .= $y_p_nm->propertyName . ' : ' . $y_p_s_f . '<br>';
        }
        ...
        foreach ($query as $tax1) {
            $y_tx1 = $tax1->total_cost / $service_fee->fee;
            $y_al_tx1_5 = $y_tx1 / 5;
    ...
    public function payments(){
        $getBookingDetails = $this->HotelBookingDetails->find('all')->where(['payment_status IN' => [3,5]]);
        $this->set(compact('getBookingDetails'));
    }
    public function refoundPayment($id = null){
        if($id){
            $this->HotelBookingDetails->updateAll(['payment_status' => 5], ['id' => $id]);
            return $this->redirect(HTTP_ROOT . 'appadmins/payments/');
        }
    }
    public function PropertyBookingOverview($id = null) {
        $this->set(compact('id'));
    }
