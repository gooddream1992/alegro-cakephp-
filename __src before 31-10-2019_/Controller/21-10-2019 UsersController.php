...
//attachment files path array
    ...
    public function myBooking() {
        ...
        if (!empty($_GET['q']) && ($_GET['q'] == 'a')) {
            $all_pro_booking = $this->HotelBookingDetails->find('all')->where(['user_id' => $this->Auth->user('id'), 'payment_status' => 3, 'check_out >=' => date('Y-m-d')])->order(['check_in' => 'DESC']);
        } elseif (!empty($_GET['q']) && ($_GET['q'] == 'p')) {
            $all_pro_booking = $this->HotelBookingDetails->find('all')->where(['user_id' => $this->Auth->user('id'), 'payment_status' => 1])->order(['check_in' => 'DESC']);
        } elseif (!empty($_GET['q']) && ($_GET['q'] == 'c')) {
            $all_pro_booking = $this->HotelBookingDetails->find('all')->where(['user_id' => $this->Auth->user('id'), 'payment_status' => 2])->order(['check_in' => 'DESC']);
        } elseif (!empty($_GET['q']) && ($_GET['q'] == 'c')) {
            $all_pro_booking = $this->HotelBookingDetails->find('all')->where(['user_id' => $this->Auth->user('id'), 'payment_status' => 3, 'check_out <=' => date('Y-m-d')])->order(['check_in' => 'DESC']);
        } else {
...
//        $activecount = $active->count();
    ...
    public function hotelSearchResult() {
        ...
        if ($this->request->is('get')) {
            ...
            $search_property = $this->ExtranetsUserPropertyLocation->find('all')->where(['city' => $locations])->extract('property_id');
            $search_property_count = $this->ExtranetsUserPropertyLocation->find('all')->where(['city' => $locations])->count();
            $intersect1 = [];
...
//                echo "already exit";
    ...
    public function ajaxHotelLocations() {
        if ($this->request->is('post')) {
            $datas = $this->request->data;
            $data = $this->HotelCountryCitys->find('all')->where(['city_name like' => '%' . $datas['origin'] . '%'])->group('city_name')->toArray();
        }
        $this->set(compact('data','datas'));
        //exit;
    }
...
//            $propertyFee1 = $this->HotelServiceFee->find('all')->where(['id' => 1])->first();
        ...
        if ($this->request->is('post')) {
            ...
            if (!empty($refer_idd)) {
                ...
                $room_type = $propertyD_email->property_type;
                ...
                $book_link = HTTP_ROOT . 'thank_you_order_hotel/' . $htl_booking_details_email->room_id . '/' . $htl_booking_details_email->property_id . '/' . $htl_booking_details_email->id;
                ...
                $this->Custom->sendEmail($to, $from, $subject, $message);
                return $this->redirect(HTTP_ROOT . 'thank_you_order_hotel/' . $id . '/' . $RmDet->property_id . '/' . $gt_htl_id->id . '?' . $dats['qury_tri']);
            } else {
    ...
    function thankYouOrderHotel($room_id = null, $pro_id = null, $id = null) {
        ...
        $this->set(compact('propertyD', 'reff', 'htl_booking_details', 'pro_id', 'id', 'room_id'));
    }
    ...
    public function order($id = null) {
        ...
        $success_return = HTTP_ROOT . 'thank_you_order_hotel/' . $id;
