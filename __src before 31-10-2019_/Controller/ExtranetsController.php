...
//
    ...
    public function pricing($id = null) {
        ...
        if ($this->request->is('post')) {
            ...
            for ($i = 1; $i <= $data['count']; $i++) {
                ...
                $data['long_days'] = @$data['long_days' . $i];
                $data['fix_price'] = @$data['fix_price' . $i];
    ...
    public function setBedrooms() {
        ...
        $newEntry->total_rooms = $data['room_count'];
        $this->ExtranetsUserPropertyBed->save($newEntry);
        // print_r($data);
...
//Check write Access to Directory
    ...
    public function bookingDetails($id = null) {
        ...
        if (!empty($_GET['token'])) {
            ...
            if ($_GET['token'] == 5) {
                $this->HotelBookingDetails->updateAll(['user_htl_reach_status' => 5,'user_htl_reach_status' => date('Y-m-d h:i:s')], ['booking_no' => $id]);
            }
            if ($_GET['token'] == 2) {
                $this->HotelBookingDetails->updateAll(['user_htl_reach_status' => 2], ['booking_no' => $id]);
            }
            if ($_GET['token'] == 1) {
...
';
        ...
        $this->HotelBookingDetails->updateAll(['check_in' => date_format(date_create(str_replace('/', '-', $data['checkIn'])), 'Y-m-d'), 'check_out' => date_format(date_create(str_replace('/', '-', $data['checkOut'])), 'Y-m-d'), 'user_htl_reach_status' => 4], ['booking_no' => $data['id']]);
        echo json_encode(['status' => 'success']);
        exit;
    }
