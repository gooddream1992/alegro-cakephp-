 ...
 <?php }else{ if (@$result_property_count != 0) { ?>
                    ...
                    <div id="main_search_data">
                        ...
                        foreach ($result_property as $list) {
                            $boostStatus = $this->User->chkBoost($list->id);
                            $htl_rm_pric = $this->User->getHotelprice($list->id, $guest);
                            $htl_fixrm_pric = $this->User->getHotelfixprice($list->id, $guest);
                            $htl_rm_pric_frea = $this->User->get_featured_fee($list->id);
                            $getBetDetails = $this->User->propertyBedCount($list->id, $guest);
                            $aminity = $this->User->propertyAmenities($list->id, $guest);
                            $longDays = $this->User->longDays($list->id);
                            if ($res < $htl_rm_pric)
                                $res = $htl_rm_pric;
                            ?>
                            <div class="hotel-lsting <?php if($boostStatus ==1 ){ ?>sponsore-list<?php } ?>">
                                ...
                                <div class="hotel-lsting-middle" style="width: 48%;">
                                    ...
                                    <p><?= $this->User->propertySize($list->id, $guest); ?> <?php echo __('Sqm') ?> <br>
                                        <i class="fas fa-users"></i> <?php
                                        //echo $list->id;

                                        echo $this->User->propertyPeople($list->id, $guest);
                                        if ($this->User->propertyPeople($list->id, $guest) <= 1) {
                                ...
                                <div class="hotel-lsting-right">
                                    <h6><?php echo $this->User->reviewCount($list->id); ?> <?php echo __('reviews') ?><span><?php echo is_nan($this->User->totalRating($list->id)) ? 0 : $this->User->totalRating($list->id); echo "/5"; ?></span></h6>
                            ...
                            <?php } ?>
                                    <div class="form-group" style="margin:0; float: right;">
                                        <label>&nbsp;</label>
                                        <a href="<?= HTTP_ROOT; ?>preview/<?= $list->id; ?>?<?= $_SERVER['QUERY_STRING']; ?>" target="_blank"><button type="button" class="btn btn-primary hvr-grow btn-fill"><?php echo __('Check Availability') ?></button></a>
                                    </div>
                                </div> 
                            </div>
