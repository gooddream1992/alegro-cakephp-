 ...
 <div class="wrapper">
  ...
  <ul>
   <?php foreach ($dataListings as $list) { ?>
      <li>
       <a href="<?php echo HTTP_ROOT . 'all-photo/' . $this->Custom->makeSeoUrl($list->name) . '-' . $list->id ?>">
        <div><img src="<?php echo HTTP_ROOT . ALBUM . $list->image ?>" alt=" image of <?php echo $list->name ?>" title="<?php echo $list->name; ?>" /></div>
