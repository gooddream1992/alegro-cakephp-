...
<section class="alavalde-pannel">
        ...
        <div class="row">
            <div class="col-sm-6 col-md-6 col-lg-6">
                <div class="Hotel-valad-left">
                    ...
                    <h6><i class="fas fa-map-marker-alt"></i> <?= @$_GET['from_location_name']; ?>, <?= $this->User->cityName(@$result_property->id); ?></h6>
            ...
            <div class="col-sm-6 col-md-6 col-lg-6">
                <div class="Hotel-valad-right">
                    ...
                    <h1>
                        ...
                        $longDays = $this->User->longDays($result_property->id);
                        $bookingtype = $this->User->getBookingRoomType($result_property->id);
                        $roomtype = $this->User->getPropertyType($result_property->id);
                        $htl_fixrm_pric = $this->User->getHotelfixprice($result_property->id, $guest);
                        //echo $htl_fixrm_pric;exit;

                        if ($dateDifference >= $longDays && !empty($longDays)) {
                            if (!empty($this->request->session()->read("CURRENCY"))) {
                                echo $first_low_price_room = $this->request->session()->read("CURRENCY") . ' ' . number_format($this->User->priceConvert($this->request->session()->read("CURRENCY"), $htl_fixrm_pric), 2);
                            } else {
                                echo $first_low_price_room = 'AOA ' . $this->User->changeFormat(number_format($htl_fixrm_pric, 2));
                            }
                        } else {
                            if (!empty($this->request->session()->read("CURRENCY"))) {
                                echo $first_low_price_room = $this->request->session()->read("CURRENCY") . ' ' . number_format($this->User->priceConvert($this->request->session()->read("CURRENCY"), $this->User->propertyPrice($result_property->id)), 2);
                            } else {
                                echo $first_low_price_room = 'AOA ' . $this->User->changeFormat(number_format($this->User->propertyPrice($result_property->id), 2));
                            }
                        }
                        ?>
                        <div class="price">
    ...
    <!--                    <a href="<?= HTTP_ROOT; ?>booking_entire/<?= $result_property->id; ?>?<?= $_SERVER['QUERY_STRING']; ?>">-->
                        <!-- #available-main-->
                        <?php if ($booking_available) { ?>
                            <a href="<?= HTTP_ROOT; ?>booking_entire/<?= $result_property->id; ?>?<?= $_SERVER['QUERY_STRING']; ?>">
                                <button class="sc-feJyhm bRPwjV sc-kkGfuU fxNDjV sc-iwsKbI hnHAyb"><div class="sc-EHOje dCPIqT" font-size="1">
                                        <?php if ($bookingtype == 1) { ?>
                                            <?php echo __('Book') ?> <?php echo __($roomtype); ?>
                                        <?php } else { ?>
                                            <?php echo __('Choose Room') ?>
                                        <?php } ?>
                ...
                <div class="gallery clearfix">
                    <div class="col col-12">
                        <div class="zoom-hover three-two">
                            ...
                            <div data-toggle="modal" data-target="#myModal"  class="Mosaic__AbsoluteFlex-z5yjyw-8 kDXaxw sc-ifAKCX jhucMM"><svg class="sc-dnqmqq hhiqKx" color="white" viewBox="0 0 24 24" width="24" height="24" fill="currentcolor"><path d="M22,16V4c0-1.1-0.9-2-2-2H8C6.9,2,6,2.9,6,4v12c0,1.1,0.9,2,2,2h12C21.1,18,22,17.1,22,16z M11,12l2,2.7l3-3.7l4,5H8L11,12z M2,6v14c0,1.1,0.9,2,2,2h14v-2H4V6H2z"></path></svg><div class="sc-EHOje kpninQ" font-size="1" color="white"><?php echo __('See all') ?> <?= !empty(count($result_property->photos) > 0) ? count($result_property->photos) : 0; ?> <?php echo __('photos') ?></div></div>
                        </div>
                    </div>
...
<section class="reasons-book">
                ...
                <div class="reasons-book-left">
                    <div class="rait-bg">
                        <div class="rait-bg-text">
                            <?php echo $this->User->totalRating($result_property->id) . '/5'; ?>
                        </div>
                        <span><?php echo __('Guest Rating') ?></span>
                    </div>
                    <div class="Overall-Score"><h6><?php echo __('Overall Score') ?><span><?php echo $this->User->totalRating($result_property->id) ?></span></h6>
                        <div class="sc-ifAKCX ikWgaA"><div class="sc-bdVaJa nTO" height="8px" width="1"><div class="BarGraph__AnimationBox-sc-1cfbrgw-0 idCQYO sc-bdVaJa bOsipX" height="8px" style="width:<?php echo $this->User->totalRating($result_property->id) * 20; ?>%;"></div></div></div>
                    </div>
                    <div class="Score-breakdown">
                        <?php
                        //echo $result_property->id;
                        echo __('Score breakdown')
                        ?> (<?php echo __('based on') ?> <span><a data-toggle="modal" data-target="#myModala" style="cursor: pointer; color: #c39e00;"> <?php echo $this->User->reviewCount($result_property->id); ?> <?php
                        if (@$reviewCount == 1) {
                            echo __('review');
                        } else {
                            echo __('reviews');
                    ...
                    <div class="clean-rait"><h6><?php echo __('CLEANLINESS') ?><span><?php echo $this->User->rating($result_property->id, 'cleanliness') ?></span></h6>
                        <div class="sc-ifAKCX ikWgaA"><div class="sc-bdVaJa nTO" height="4px" >
                                <div class="BarGraph__AnimationBox-sc-1cfbrgw-0 fqejTW sc-bdVaJa kViweL" height="4px" style="width:<?php echo $this->User->rating($result_property->id, 'cleanliness') * 20; ?>%;"></div>

                            </div></div>
                    </div>

                    <div class="staff-rait"><h6><?php echo __('STAFF') ?><span><?php echo $this->User->rating($result_property->id, 'staff') ?></span></h6>
                        <div class="sc-ifAKCX ikWgaA"><div class="sc-bdVaJa nTO" height="4px" >
                                <div class="BarGraph__AnimationBox-sc-1cfbrgw-0 fqejTW sc-bdVaJa kViweL" height="4px" style="width:<?php echo $this->User->rating($result_property->id, 'staff') * 20; ?>%;"></div>

                            </div></div>
                    </div>

                    <div class="staff-rait"><h6><?php echo __('LOCATION') ?><span><?php echo $this->User->rating($result_property->id, 'location') ?></span></h6>
                        <div class="sc-ifAKCX ikWgaA"><div class="sc-bdVaJa nTO" height="4px" >
                                <div class="BarGraph__AnimationBox-sc-1cfbrgw-0 fqejTW sc-bdVaJa kViweL" height="4px" style="width:<?php echo $this->User->rating($result_property->id, 'location') * 20; ?>%;"></div>
                        ...
                        <div class="modal-body" style="padding: 0;">
                            ...
                            <div class="modal-header">
                                ...
                                <div class="d-table-cell">
                                    <select name="" onchange="getShort(this.value, '<?php echo $result_property->id ?>')" id="sortBy" class="sortBy" style="display: none;">
                                    ...
                                    <div class="nice-select sortBy" tabindex="0">
                                        <span class="current"><?= __("All Reviews"); ?></span>
                                        <ul class="list">
                                            <li data-value="lth" class="option selected focus"><?= __("All Reviews"); ?></li>
                                            <li data-value="htl" class="option"><?= __("Latest"); ?></li>
                                            <li data-value="str" class="option"><?= __("Oldest"); ?></li>
                                            <li data-value="rev" class="option"><?= __("Stars"); ?></li>
                                        </ul></div>
                                    <i class="fa fa-angle-down formIconSort"></i>
                                </div>
                            </div>
                            <div class="review-page-left" id='all_rivew'>
                                <style>
                                    .checked {
                                        color: orange;
                                    }
                                ...
                                <?php
                                $allReviewGet = $this->User->allReview($result_property->id, 'all');
                                ...
                                foreach ($allReviewGet as $rv) {
                                    ...
                                    <div class="review-b-le">
                                        ...
                                        <h5><?php echo $rv->user_firstname . ' ' . $rv->user_lastname ?></h5> <span style="color: #000;"><?php echo date_format($rv->review_date, 'd M Y'); ?></span>
                                        <div style="width: 28%;float: left;margin: 0px 0px 7px 17px;">
    <!--                                            <i class="fa fa-star"></i>
                                            ...
                                            <span style="color: #000000;padding: 4px 4px;margin: 0px 7px;font-size: 15px;font-weight: bold;background: #f9d749;border: 1px solid #f9d749;"> <?php echo $this->User->cReview($rv->id) ?>/5</span>
                                        </div>
            ...
            <div class="col-sm-6 col-md-6 col-lg-6">
                ...
                <div class="reasons-book-right" style="margin-top: 25px;">
                    <h4><i class="fas fa-home"></i> <?= __("Entire Place"); ?></h4>
                    <!--                    <div id='less-content'>
                                            <p><?= substr(nl2br($result_property->description->describeYourPlace), 0, 705); ?></p>
                                            <div><span onclick="$('#less-content').hide();$('#more-content').show();">Read More...</span></div>
                                        </div>
                    ...
                    <div class='comment more' >
                        <?= __("You will have this property for yourself, your family as well as your friends and will not share with anyone else."); ?>

                    </div>
                </div>

                <div class="reasons-book-right" style="margin-top: 25px;">
                    <h4><i class="fas fa-piggy-bank"></i> <?= __("Cancellation Policy"); ?> :: <?= __($this->User->getCancelationEntireProperty($result_property->id)); ?> </h4>
                    <div class="cancellation" >
                        <?php
                        if ($this->User->getCancelationEntireProperty($result_property->id) == "Flexible") {
                        ...
                        } elseif ($this->User->getCancelationEntireProperty($result_property->id) == "Moderate") {
                        ...
                        } elseif ($this->User->getCancelationEntireProperty($result_property->id) == "Strict") {
            ...
            <div class="row">
                ...
                <div class="col-sm-12 col-md-12 col-lg-12" id="topClass" >
                    ...
                    foreach ($this->User->getAllRooms($id) as $htl_rm) {
                        //pj($htl_rm);
                        $property_availablity = $this->User->checkRoomAvailable($htl_rm->id, @$_GET['rooms'], @$_GET['hotel_check_in'], @$_GET['from_location_name'], @$_GET['hotel_check_out']);
                        $property_availablity_date = $this->User->checkRoomAvailableDate($htl_rm->id, @$_GET['rooms'], @$_GET['hotel_check_in'], @$_GET['from_location_name']);
                        ...
                        $avail = $this->User->getAvailability(0);
                        ?>
                                        ...
                                        <div class="best-match-left">
                                            ...
                                            <ul>
                                                <li><span><i class="fas fa-bed"></i></span>
                                                    ...
                                                    <?php
                                                    $sbdn = '';
                                                    if (!empty(@$this->User->getSubbed(@$htl_rm->user_id, @$htl_rm->property_id, @$htl_rm->id))) {
...
//                                                        }
    ...
    <!--                                                    <div class="Best-Deal"><a href="#"><i class="fa fa-dollar"></i>Best Deal</a></div>-->
                                                <?php $bookingtypex = $this->User->getBookingRoomType($htl_rm->property_id); ?>
                                                <?php
                                                if ($bookingtypex == 2) {

                                                    if (($property_availablity == "Booing_Available") && ($property_availablity_date == "Booing_Available")) {
                                                        //if ($htl_rm->room_count >= $_GET['rooms']) {
                                                        ?>
                                                        ...
                                                        <div class="non-refund-price" style="float:right">
                                                            <h5 class="old-price">
                                                                <?php if ($htl_rm_pric_frea > 0) { ?>
                                                                    <span><?php echo __('was') ?></span><span>
                                                                        <?php
                                                                        if (!empty($this->request->session()->read("CURRENCY"))) {
                                                                            echo $this->request->session()->read("CURRENCY") . ' ' . number_format($this->User->priceConvert($this->request->session()->read("CURRENCY"), ($htl_rm->pricing->price_main)), 2) . '/' . __('night');
                                                                        } else {
                                                                            echo 'AOA ' . $this->User->changeFormat(number_format($htl_rm->pricing->price_main, 2)) . '/' . __('night');
                                                                        }
                                                                        ?>                 </span>
                                                                    <?php
                                                                } else {
                                                            ...
                                                            <h5 class="new-price">
                                                                <?php
                                                                $htl_rm_pric = $htl_rm->pricing->price_main;
                                                                $htl_fixrm_pric1 = $htl_rm->pricing->fix_price;

                                                                $cal001 = ($htl_rm_pric - ($htl_rm_pric * ($htl_rm_pric_frea / 100)));
                                                                $cal002 = $cal004 = $dateDifference * $htl_fixrm_pric1;
                                                                $cal003 = $dateDifference * ($htl_rm_pric - ($htl_rm_pric * ($htl_rm_pric_frea / 100)));
                                                                $cal005 = $dateDifference * $htl_rm_pric;
                                                                ...
                                                                if ($htl_rm_pric_frea > 0) {
                                                                    ...
                                                                    if ($dateDifference >= $longDays && !empty($longDays)) {
                                                                        if (!empty($this->request->session()->read("CURRENCY"))) {
                                                                            echo $this->request->session()->read("CURRENCY") . ' ' . number_format($this->User->priceConvert($this->request->session()->read("CURRENCY"), $htl_fixrm_pric1), 2) . '/' . __('night');
                                                                        } else {
                                                                            echo 'AOA ' . $this->User->changeFormat(number_format($htl_fixrm_pric1, 2)) . '/' . __('night');
                                                                        }
                                                                    } else {

                                                                        if (!empty($this->request->session()->read("CURRENCY"))) {
                                                                            echo $this->request->session()->read("CURRENCY") . ' ' . number_format($this->User->priceConvert($this->request->session()->read("CURRENCY"), $cal001), 2) . '/' . __('night');
                                                                        } else {
                                                                            echo 'AOA ' . $this->User->changeFormat(number_format($cal001, 2)) . '/' . __('night');
                                                                        }
                                                                    }
                                                                    ?>
                                                                <?php } else { ?>
                                                            ...
                                                            <p class="text text-success" style="font-weight: normal;font-size: 15px;">
                                                                <?php if ($htl_rm_pric_frea > 0) { ?>
                                                                    ...
                                                                    if ($dateDifference >= $longDays && !empty($longDays)) {
                                                                        ...
                                                                        if (!empty($this->request->session()->read("CURRENCY"))) {
                                                                            echo $this->request->session()->read("CURRENCY") . ' ' . number_format($this->User->priceConvert($this->request->session()->read("CURRENCY"), $cal002), 2);
                                                                            ?> <?= ' ' . __("for"); ?> <?= $dateDifference; ?><?php
                                                                        } else {
                                                                            echo 'AOA ' . $this->User->changeFormat(number_format($cal002, 2));
                                                                            ?> <?= ' ' . __("for"); ?> <?= $dateDifference; ?><?php
                                                                        }
                                                                    } else {

                                                                        if (!empty($this->request->session()->read("CURRENCY"))) {
                                                                            echo $this->request->session()->read("CURRENCY") . ' ' . number_format($this->User->priceConvert($this->request->session()->read("CURRENCY"), $cal003), 2);
                                                                            ?> <?= ' ' . __("for"); ?> <?= $dateDifference; ?><?php
                                                                        } else {
                                                                            echo 'AOA ' . $this->User->changeFormat(number_format($cal003, 2));
                                                                            ?> <?= ' ' . __("for"); ?> <?= $dateDifference; ?><?php
                                                                        }
                                                                        ?>
                                                                    <?php } ?>
                                                                ...
                                                                <?php } else { ?>
                                                                    ...
                                                                    if ($dateDifference >= $longDays && !empty($longDays)) {
                                                                        ...
                                                                        if (!empty($this->request->session()->read("CURRENCY"))) {
                                                                            echo $this->request->session()->read("CURRENCY") . ' ' . number_format($this->User->priceConvert($this->request->session()->read("CURRENCY"), $cal004), 2);
                                                                            ?><?= ' ' . __("for"); ?> <?= $dateDifference; ?><?php
                                                                        } else {
                                                                            echo 'AOA ' . $this->User->changeFormat(number_format($cal004, 2));
                                                                            ?> <?= ' ' . __("for"); ?> <?= $dateDifference; ?><?php
                                                                        }
                                                                    } else {

                                                                        if (!empty($this->request->session()->read("CURRENCY"))) {
                                                                            echo $this->request->session()->read("CURRENCY") . ' ' . number_format($this->User->priceConvert($this->request->session()->read("CURRENCY"), $cal005), 2);
                                                                            ?> <?= ' ' . __("for"); ?> <?= $dateDifference; ?> <?php
                                                                        } else {
                                                                            echo 'AOA ' . $this->User->changeFormat(number_format($cal005, 2));
                                                                            ?> <?= ' ' . __("for"); ?> <?= $dateDifference; ?> <?php
                                                                        }
                                                                        ?><?php
                                                                    }
                                                        ...
                                                        <div class="non-refund-right"  style="float:right">
                                                            <a href="<?= HTTP_ROOT; ?>booking_entire/<?= $htl_rm->id; ?>?<?= $_SERVER['QUERY_STRING']; ?>" target="_blank"><?php echo __('Book') ?></a>
                                                        </div>
                                                    <?php } else { ?>
...
//$random_keys=array_rand($a,2);
                        ...
                        <div class="Similar-right-box">
                            <div class="Hotel-Alvalade">
                                ...
                                <div class="sc-eMRERa hAZDpl sc-bdVaJa cDiPQh"><div class="sc-EHOje hSElwR" font-size="1" color="black"><?php echo is_nan($this->User->totalRating($result_property->id)) ? 0 : $this->User->totalRating($result_property->id); ?>/5</div></div>
                                <div class="sc-eMRERa hAZDpl sc-bdVaJa cDiPQh"><div class="sc-fAjcbJ dtkclf sc-EHOje hSElwR" font-size="1" color="black"><?= $this->User->cityName(@$result_property->id); ?></div></div>

                                <div class="sc-ileJJU gbSOWI sc-bdVaJa kRGLvU"><div class="sc-EHOje tkrSK" color="green" font-size="2">
                                        <?php
                                        if (!empty($this->request->session()->read("CURRENCY"))) {
                                            echo $this->request->session()->read("CURRENCY") . ' ' . number_format($this->User->priceConvert($this->request->session()->read("CURRENCY"), $this->User->propertyPrice($result_property->id)), 2);
                                        } else {
                                            echo 'AOA ' . $this->User->changeFormat(number_format($this->User->propertyPrice($result_property->id), 2));
                                        }
                                        ?>
                            ...
                            if (!empty($iOne)) {
                                ...
                                if (count($iOne) >= 5) {
                                    ...
                                    $rel_pro_count_max = count($iOne);
                                }
                                foreach ($iOne as $relatedOne) {
                                    if ($rel_pro_count <= $rel_pro_count_max) {
                                        $sndHotel = $this->User->getDataForHotel($relatedOne);
                                        unset($sndImgUrl);
                                        $sndImgUrl = "";
                                        ...
                                        <?php if (!empty($sndImgUrl)) { ?>
                                            <style>
                                                .Hotel-Trópico-img<?= $relatedOne; ?>{
                                                    background: url(<?= HTTP_ROOT . 'img/pro_pic/' . $sndImgUrl; ?>)no-repeat !important;
                                        ...
                                        <div class="Hotel-Trópico">
                                            <div class="Hotel-Trópico-img Hotel-Trópico-img<?= $relatedOne; ?>">
                                                <div class="sc-fdQOMr gkGSap sc-bdVaJa cZxsvx">
                                                    <div class="sc-ifAKCX erTWuZ">
                                                    </div>
                                                </div>
                                            ...
                                            <div class="sc-eMRERa hAZDpl sc-bdVaJa cDiPQh"><div class="sc-EHOje hSElwR" font-size="1" color="black"><?php echo is_nan($this->User->totalRating($relatedOne)) ? 0 : $this->User->totalRating($relatedOne); ?>/5</div></div>
                                            <div class="sc-eMRERa hAZDpl sc-bdVaJa cDiPQh"><div class="sc-fAjcbJ dtkclf sc-EHOje hSElwR" font-size="1" color="black"><?= $sndHotel->location->city; ?></div></div>

                                            <div class="sc-ileJJU gbSOWI sc-bdVaJa kRGLvU"><div class="sc-EHOje tkrSK" color="green" font-size="2">

                                                    <?php
                                                    if (!empty($this->request->session()->read("CURRENCY"))) {
                                                        echo $this->request->session()->read("CURRENCY") . ' ' . number_format($this->User->priceConvert($this->request->session()->read("CURRENCY"), $this->User->propertyPrice($sndHotel->id)), 2);
                                                    } else {
                                                        echo 'AOA ' . $this->User->changeFormat(number_format($this->User->propertyPrice($sndHotel->id), 2));
                                                    }
                                                    ?>

                                                </div></div>

                                            <div class="sc-bdVaJa fJntgG"><a href="<?= HTTP_ROOT; ?>users/preview_entire/<?= $relatedOne; ?>?<?= $_SERVER['QUERY_STRING']; ?>"><button class="sc-cNQqM eKqHMe sc-iwsKbI jHpzKw" data-autobot-element-id="HTL_DETAIL_SIMILAR_HOTEL_SELECT"><?php echo __('Select') ?></button></a></div>
                                        </div>
                                        <?php
                                        $rel_pro_count++;
                                    }
                ...
                <div class="map-part" id="map-part">
                    <div class="map-text">
                        <h3><?php
                            ...
                            ?></h3>
                        <h5><?= @$_GET['from_location_name']; ?>, <?= $this->User->cityName(@$result_property->id); ?></h5>
                    </div>
        ...
        <div class="row">
            ...
            <div class="col-sm-12 col-md-12 col-lg-12">
                <div class="guest-part-left">
                    <span class="sc-hGoxap jItMmH" data-autobot-element-id="HTL_DETAIL_REVIEWS_SCORES_BOTTOM" data-unit-id="HTL_DETAIL_REVIEWS_SCORES_BOTTOM"><?php echo $this->User->totalRating($result_property->id); ?></span>
                    <div class="sc-EHOje iMnUJL" color="blue">
                        <?php if ($this->User->totalRating($result_property->id) >= 1) { ?>
                            <?php echo __('Poor'); ?>
                        <?php } else if ($this->User->totalRating($result_property->id) >= 2) { ?>
                            <?php echo __('Fair'); ?>
                        <?php } else if ($this->User->totalRating($result_property->id) >= 3) { ?>
                            <?php echo __('Good'); ?>
                        <?php } else if ($this->User->totalRating($result_property->id) >= 4) { ?>
                            <?php echo __('Excellent'); ?>
                        <?php } else if ($this->User->totalRating($result_property->id) >= 5) { ?>
                            <?php echo __('WOW'); ?>
                        <?php } ?>
                    </div>
                </div>
                    ...
                    <ul class="sc-fjmCvl OFVLp">
                        <li class="sc-TFwJa dnayhP">
                            ...
                            <span class="sc-krDsej irQgW sc-bZQynM bgdexZ">
                                <span class="sc-dTdPqK eYrhVC sc-bZQynM kaWqEv" style='width:<?php echo $this->User->rating($result_property->id, 'cleanliness') * 20; ?>%'></span>
                                <span class="sc-itybZL bPIPWU"><?php echo $this->User->rating($result_property->id, 'cleanliness') ?></span></span></li>
                        <li class="sc-TFwJa dnayhP">
                            <span class="sc-bHwgHz irfDts"><?= __("STAFF"); ?></span>
                            <span class="sc-krDsej irQgW sc-bZQynM bgdexZ">
                                <span class="sc-dTdPqK jlGsdO sc-bZQynM kaWqEv" style='width:<?php echo $this->User->rating($result_property->id, 'staff') * 20; ?>%'></span>
                                <span class="sc-itybZL bPIPWU"><?php echo $this->User->rating($result_property->id, 'staff') ?></span></span>
                        </li>
                        <li class="sc-TFwJa dnayhP"><span class="sc-bHwgHz irfDts"><?= __("LOCATION"); ?></span>
                            <span class="sc-krDsej irQgW sc-bZQynM bgdexZ">
                                <span class="sc-dTdPqK eYrhVC sc-bZQynM kaWqEv" style='width:<?php echo $this->User->rating($result_property->id, 'location') * 20; ?>%'></span>
                                <span class="sc-itybZL bPIPWU"><?php echo $this->User->rating($result_property->id, 'location') ?></span></span></li></ul>
                </div>
                <div class="Score-breakdown"><?= __("based on"); ?> <span><a  data-toggle="modal" data-target="#myModala" style="cursor: pointer; color: #c39e00;"><?php echo $this->User->reviewCount($result_property->id); ?> <?php
                            if (@$reviewCount == 1) {
                                echo __('review');
                            } else {
                                echo __('reviews');
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <ul style="list-style: none;padding: 10px;line-height: 24px;color: rgb(104, 123, 142);font-weight: 500;">
                    <?php
                    if (!empty(json_decode($result_property->amenity->Top))) {
                        foreach (json_decode($result_property->amenity->Top) as $val) {
                            ?>
                            <li>
                                <svg class="sc-dnqmqq bFZHNc" color="green" viewBox="0 0 24 24" width="16" height="16" fill="currentcolor"><path d="M8.6,15.6l-4.2-4.2L3,12.8l5.6,5.6l12-12L19.2,5L8.6,15.6z"></path></svg>
...
<script>
    function getShort(value, propertyId) {
        var pageurl = '<?php echo HTTP_ROOT ?>';
        jQuery.ajax({
            type: "POST",
            url: pageurl + "users/hotelAjaxReview",
            dataType: 'html',
            data: {short: value, propertyId: propertyId},
            success: function(res) {
